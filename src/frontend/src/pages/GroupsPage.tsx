...
function GroupsPage() {
  ...
  return <section className="groups-page">
    ...
    {groups.length === 0 ? <div className="groups-empty"><h2>Chưa có nhóm nào.</h2><p>Tạo nhóm học tập hoặc cầu lông để bắt đầu đồng bộ lịch và hoạt động.</p><button className="button button--primary" onClick={() => setCreateOpen(true)}>Tạo nhóm đầu tiên</button></div> : <div className="group-grid">{groups.map((group) => <Link className={`workspace-card workspace-card--${group.type.toLowerCase()}`} to={`/groups/${group.id}`} key={group.id}><span className="workspace-card__mark" aria-hidden="true">{group.type === 'BADMINTON' ? <Trophy size={24} strokeWidth={1.8} /> : <BookOpen size={24} strokeWidth={1.8} />}</span><div className="workspace-card__meta"><span>{group.type === 'BADMINTON' ? 'Cầu lông' : 'Học tập'}</span><span>{group.role}</span></div><h2>{group.name}</h2><p>{group.description || (group.type === 'BADMINTON' ? 'Quản lý buổi chơi, thành viên và kết quả.' : 'Lập kế hoạch học, mục tiêu và tài liệu chung.')}</p><b>Mở không gian <span aria-hidden="true">→</span></b></Link>)}</div>}
