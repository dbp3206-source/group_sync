...
function LoginPage() {
  const { connectionState, login, retryBootstrap } = useAuth()
  ...
  const waking = connectionState === 'WAKING'
    ...
    <div className="auth-card-wrap">
      <div className="auth-page">
        ...
        <form className="page-panel form-stack" onSubmit={submit}>
          ...
          {waking && <div className="status-card status-card--info" role="status"><strong>KnowledgeOS đang khởi động máy chủ…</strong><span>Bản demo có thể cần vài giây để khởi động lại sau thời gian không sử dụng.</span></div>}
          {unavailable && !saving && <div className="status-card status-card--error" role="alert"><strong>KnowledgeOS chưa thể kết nối máy chủ.</strong><span>Vui lòng thử lại khi máy chủ demo đã thức dậy.</span><button type="button" className="button button--quiet" disabled={retrying} onClick={() => void retryConnection()}>{retrying ? 'Đang kiểm tra…' : 'Thử lại kết nối'}</button></div>}
          ...
          <button className="button button--primary" disabled={saving}>{saving ? (waking ? 'KnowledgeOS đang khởi động máy chủ…' : 'Đang đăng nhập…') : 'Đăng nhập'}</button>
          {saving && (
            <p style={{ fontSize: '0.82rem', color: 'var(--kos-muted)', textAlign: 'center', margin: '0.5rem 0 0' }}>
              Đang xác thực với máy chủ. Nếu bản demo vừa thức dậy, quá trình có thể mất vài giây.
            </p>
          )}
        </form>
