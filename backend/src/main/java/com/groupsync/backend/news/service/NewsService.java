...
public class NewsService {
    ...
    @Transactional(readOnly = true) public List<News> list(AuthenticatedUser actor, Long groupId) { requireMember(groupId, actor.getId()); return newsRepository.findByGroupIdOrderByCreatedAtDesc(groupId).stream().map(n -> new News(n.getId(), n.getType().name(), n.getTitle(), n.getContent(), sourceId(n.getSourceKey()), n.getCreatedAt())).toList(); }
    ...
    private Long sourceId(String sourceKey) { if (sourceKey == null || !sourceKey.startsWith("MATCH_RESULT:")) return null; try { return Long.valueOf(sourceKey.substring("MATCH_RESULT:".length())); } catch (NumberFormatException ignored) { return null; } }
