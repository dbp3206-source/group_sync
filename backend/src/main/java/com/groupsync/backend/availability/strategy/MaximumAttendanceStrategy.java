...
@Component("maximumAttendanceStrategy")
public class MaximumAttendanceStrategy implements SchedulingStrategy {
    @Override
    public List<AvailabilityCandidate> rank(List<AvailabilityCandidate> candidates, int minimumAttendance) {
        return candidates.stream()
            ...
            .sorted(Comparator.comparingInt(AvailabilityCandidate::attendance).reversed().thenComparing(AvailabilityCandidate::start))
            .toList();
    }
}
